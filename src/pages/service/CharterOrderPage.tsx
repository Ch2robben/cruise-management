  ...
  return (
    <div>
      ...
      <SearchPanel onSearch={() => fetchData(1)} onReset={handleReset} loading={loading}>
        ...
        <div className="flex flex-col gap-1.5"><label className="text-xs text-gray-500">预约类型</label><select value={typeFilter} onChange={(event) => setTypeFilter(event.target.value)} className="w-32 select-field"><option value="all">全部</option>{Object.entries(reservationTypeLabels).map(([key, label]) => <option key={key} value={key}>{label}</option>)}</select></div>
        <div className="flex flex-col gap-1.5"><label className="text-xs text-gray-500">订单状态</label><select value={statusFilter} onChange={(event) => setStatusFilter(event.target.value)} className="w-36 select-field"><option value="all">全部</option><option value="pending_accept">待接单</option><option value="accepted">已接单</option><option value="signed">已签约</option><option value="in_progress">执行中</option><option value="completed">已完成</option><option value="cancelled">已取消</option></select></div>
        ...
        <div className="space-y-5">
          <div>
            ...
            <div className="grid grid-cols-3 gap-4">
              <div><label className="block text-sm text-gray-700 mb-1">预约类型 <span className="text-red-500">*</span></label><select value={form.reservationType} onChange={(event) => setForm({ ...form, reservationType: event.target.value as CharterReservationType })} className="w-full select-field">{Object.entries(reservationTypeLabels).map(([key, label]) => <option key={key} value={key}>{label}</option>)}</select></div>
              ...
              <div><label className="block text-sm text-gray-700 mb-1">航线</label><select value={form.routeId} onChange={(event) => setForm({ ...form, routeId: event.target.value })} className="w-full select-field">{routes.slice(0, 18).map((route) => <option key={route.id} value={route.id}>{route.name}</option>)}</select></div>
              <div><label className="block text-sm text-gray-700 mb-1">船型</label><select value={form.shipId} onChange={(event) => setForm({ ...form, shipId: event.target.value })} className="w-full select-field">{ships.map((ship) => <option key={ship.id} value={ship.id}>{ship.name}</option>)}</select></div>
              <div><label className="block text-sm text-gray-700 mb-1">计费类型</label><select value={form.billingType} onChange={(event) => setForm({ ...form, billingType: event.target.value as CharterBillingType })} className="w-full select-field">{Object.entries(billingTypeLabels).map(([key, label]) => <option key={key} value={key}>{label}</option>)}</select></div>
            </div>
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y">
                {form.feeItems.map((item) => (
                  <tr key={item.id}>
                    <td className="px-3 py-2"><select value={item.item} onChange={(event) => updateFeeItem(item.id, 'item', event.target.value)} className="w-full select-field-sm"><option value="船票费">船票费</option><option value="茶水费">茶水费</option><option value="餐饮费">餐饮费</option><option value="服务费">服务费</option><option value="场地费">场地费</option><option value="其他">其他</option></select></td>
          ...
          <div>
            ...
            <div className="grid grid-cols-3 gap-4">
              ...
              <div><label className="block text-sm text-gray-700 mb-1">尾款结算方式</label><select value={form.settlementType} onChange={(event) => setForm({ ...form, settlementType: event.target.value as CharterSettlementType })} className="w-full select-field">{Object.entries(settlementTypeLabels).map(([key, label]) => <option key={key} value={key}>{label}</option>)}</select></div>
            </div>
          </div>
