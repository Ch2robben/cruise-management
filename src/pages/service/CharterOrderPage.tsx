...
const reservationTypeOptions: SelectOption<CharterReservationType>[] = Object.entries(reservationTypeLabels).map(([value, label]) => ({
  value: value as CharterReservationType,
  label,
}))

const billingTypeOptions: SelectOption<CharterBillingType>[] = Object.entries(billingTypeLabels).map(([value, label]) => ({
  value: value as CharterBillingType,
  label,
}))

const settlementTypeOptions: SelectOption<CharterSettlementType>[] = Object.entries(settlementTypeLabels).map(([value, label]) => ({
  value: value as CharterSettlementType,
  label,
}))

const reservationTypeFilterOptions: SelectOption<string>[] = [
  { value: 'all', label: '全部' },
  ...reservationTypeOptions,
]

const charterStatusOptions: SelectOption<string>[] = [
  { value: 'all', label: '全部' },
...
const routeOptions: SelectOption<string>[] = routes.slice(0, 18).map((route) => ({ value: route.id, label: route.name }))
const shipOptions: SelectOption<string>[] = ships.map((ship) => ({ value: ship.id, label: ship.name }))

const feeItemOptions: SelectOption<string>[] = [
  { value: '船票费', label: '船票费' },
  { value: '茶水费', label: '茶水费' },
  { value: '餐饮费', label: '餐饮费' },
  { value: '服务费', label: '服务费' },
  ...
  return (
    <div>
      ...
      <SearchPanel onSearch={() => fetchData(1)} onReset={handleReset} loading={loading}>
        ...
        <div className="flex flex-col gap-1.5"><label className="text-xs text-gray-500">预约类型</label><SelectField value={typeFilter} onChange={setTypeFilter} options={reservationTypeFilterOptions} className="w-32" /></div>
        <div className="flex flex-col gap-1.5"><label className="text-xs text-gray-500">订单状态</label><SelectField value={statusFilter} onChange={setStatusFilter} options={charterStatusOptions} className="w-36" /></div>
        ...
        <div className="space-y-5">
          <div>
            ...
            <div className="grid grid-cols-3 gap-4">
              <div><label className="block text-sm text-gray-700 mb-1">预约类型 <span className="text-red-500">*</span></label><SelectField value={form.reservationType} onChange={(reservationType) => setForm({ ...form, reservationType })} options={reservationTypeOptions} className="w-full" /></div>
              ...
              <div><label className="block text-sm text-gray-700 mb-1">航线</label><SelectField value={form.routeId} onChange={(routeId) => setForm({ ...form, routeId })} options={routeOptions} className="w-full" /></div>
              <div><label className="block text-sm text-gray-700 mb-1">船型</label><SelectField value={form.shipId} onChange={(shipId) => setForm({ ...form, shipId })} options={shipOptions} className="w-full" /></div>
              <div><label className="block text-sm text-gray-700 mb-1">计费类型</label><SelectField value={form.billingType} onChange={(billingType) => setForm({ ...form, billingType })} options={billingTypeOptions} className="w-full" /></div>
            </div>
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y">
                {form.feeItems.map((item) => (
                  <tr key={item.id}>
                    <td className="px-3 py-2"><SelectField value={item.item} onChange={(value) => updateFeeItem(item.id, 'item', value)} options={feeItemOptions} className="w-full" size="sm" /></td>
          ...
          <div>
            ...
            <div className="grid grid-cols-3 gap-4">
              ...
              <div><label className="block text-sm text-gray-700 mb-1">尾款结算方式</label><SelectField value={form.settlementType} onChange={(settlementType) => setForm({ ...form, settlementType })} options={settlementTypeOptions} className="w-full" /></div>
            </div>
          </div>
