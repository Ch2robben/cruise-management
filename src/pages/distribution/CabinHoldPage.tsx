  ...
  return (
    <div>
      ...
      <SearchPanel onSearch={() => fetchData(1)} onReset={handleReset} loading={loading}>
        ...
        <div className="flex flex-col gap-1.5"><label className="text-xs text-gray-500">经销商</label><select value={dealerFilter} onChange={(event) => setDealerFilter(event.target.value)} className="w-44 px-3 py-2 border border-gray-300 rounded-lg text-sm"><option value="all">全部</option>{activeDealers.map((dealer) => <option key={dealer.id} value={dealer.id}>{dealer.name}</option>)}</select></div>
        <div className="flex flex-col gap-1.5"><label className="text-xs text-gray-500">航线</label><select value={routeFilter} onChange={(event) => setRouteFilter(event.target.value)} className="w-44 px-3 py-2 border border-gray-300 rounded-lg text-sm"><option value="all">全部</option>{routeOptions.map((route) => <option key={route} value={route}>{route}</option>)}</select></div>
        <div className="flex flex-col gap-1.5"><label className="text-xs text-gray-500">锁舱状态</label><select value={statusFilter} onChange={(event) => setStatusFilter(event.target.value)} className="w-32 px-3 py-2 border border-gray-300 rounded-lg text-sm"><option value="all">全部</option><option value="effective">有效</option><option value="released">已释放</option><option value="expired">已逾期</option></select></div>
        ...
        <div className="space-y-5">
          <div>
            ...
            <div className="grid grid-cols-3 gap-4">
              <div><label className="block text-sm text-gray-700 mb-1">经销商 <span className="text-red-500">*</span></label><select value={form.dealerId} onChange={(event) => setForm({ ...form, dealerId: event.target.value })} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm">{activeDealers.map((dealer) => <option key={dealer.id} value={dealer.id}>{dealer.name}</option>)}</select></div>
              <div><label className="block text-sm text-gray-700 mb-1">产品 <span className="text-red-500">*</span></label><select value={form.productId} onChange={(event) => setForm({ ...form, productId: event.target.value, releaseDeadline: form.releaseDeadline || '2026-05-20' })} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm">{products.slice(0, 18).map((product) => <option key={product.id} value={product.id}>{product.name}</option>)}</select></div>
              ...
              <div><label className="block text-sm text-gray-700 mb-1">舱位类型</label><select value={form.cabinType} onChange={(event) => setForm({ ...form, cabinType: event.target.value })} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm"><option value="套房">套房</option><option value="阳台房">阳台房</option><option value="海景房">海景房</option><option value="内舱房">内舱房</option></select></div>
