...
export type CruiseBookingWizardMode = 'dealer' | 'dispatch'

export interface CruiseBookingWizardProps {
  title: string
  description: string
  mode?: CruiseBookingWizardMode
  /** 计调场景：顶部附加区（如选择分销商） */
  headerExtra?: ReactNode
  /** 外层容器 class，分销台与运营端布局 padding 不同 */
  className?: string
}

export default function CruiseBookingWizard({
  title,
  description,
  mode = 'dealer',
  headerExtra,
  className = 'space-y-5 p-6 pb-20',
}: CruiseBookingWizardProps) {
  ...
  return (
    <div className={className}>
      ...
      <div className="overflow-hidden rounded-lg border border-gray-200 bg-white">
        <div className="border-b border-gray-100 bg-gray-50 px-5 py-3">
          <div className="flex flex-wrap items-center justify-between gap-3">
            ...
            <div className="text-xs text-gray-500">仅可回看已完成步骤，避免跨步骤修改导致数据不一致</div>
          </div>
        </div>
              ...
              return (
                <div key={index} className="flex items-center">
                    ...
                    <div className="min-w-0">
                      <div
                        className={`truncate text-sm font-medium ${
                          isActive ? 'text-blue-700' : isDone ? 'text-green-700' : 'text-gray-700'
                        }`}
                      >
                        {step}
                      </div>
                  ...
                  {index < steps.length - 1 && (
                    <ChevronRight className="mx-2 h-4 w-4 shrink-0 text-gray-300" />
                  )}
                </div>
              )
            })}
      ...
      <div>
        ...
        {currentStep === 1 && (
          <Step2RoomReserve
            voyageSummary={bookingDraft.voyageSummary}
            onNext={handleNext}
            onPrev={handlePrev}
          />
        )}
        ...
        {currentStep === 3 && (
          <Step4OrderConfirm data={bookingDraft} onNext={() => handleNext()} onPrev={handlePrev} />
        )}
        {currentStep === 4 && (
          <Step5DepositPayment onNext={() => handleNext()} onPrev={handlePrev} />
        )}
        {currentStep === 5 && <Step6Complete mode={mode} />}
      </div>
    </div>
  )
}
